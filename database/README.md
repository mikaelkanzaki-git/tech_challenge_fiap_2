...
For the Supabase connection, copy `.env.example` to `.env` and replace `<YOUR-PASSWORD>` locally. Keep the final database path as `/tech_challenge_fiap_2`. Do not commit `.env`.
